...
def create_new_domain(tranco, dnscharset, check_availability=False):
...
def generate_domains(nr_domains, seed=42):
    np.random.seed(seed)
    tranco = pd.read_csv('dga_classifier/data/tranco1M.csv', names=['ranking', 'domain'])
    # create 1000 valid domains
    domains = []
    print('Generating charbot domains')
    dnscharset = [chr(x) for x in range(0x61, 0x61 + 26)] + [chr(x) for x in range(0x30, 0x30 + 10)] + ['-']
    for index, _ in enumerate(range(nr_domains)):
        # Can take some time, as will query almost 1000 non existing domains
        try:
            dga = create_new_domain(tranco, dnscharset, check_availability=True)
            if dga[2] == 1:
                domains.append(dga[1])

            print(index, nr_domains, dga)
...
if __name__ == '__main__':
    ...
    if False:     # <---- to False to avoid download tranco every time
        try:
